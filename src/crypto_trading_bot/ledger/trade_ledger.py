...
def _kraken_get_balance_retry(asset: str):
    """Retry wrapper around Kraken balance fetch."""

    return kraken_get_balance(asset)
...
class TradeLedger:
    ...
    def _current_max_drawdown(self, *, sample: int = 500) -> float:
        """Return absolute drawdown from recent closed trades."""

        if not self.trades and os.path.exists(TRADES_LOG_PATH):
            self.reload_trades()

        rois: list[float] = []
        for trade in self.trades[-sample:]:
    ...
    def _guard_drawdown_limit(self, *, limit: float = 0.10) -> None:
        """Raise if cumulative drawdown breaches ``limit``."""

        drawdown = self._current_max_drawdown()
        if drawdown < limit:
            self._drawdown_guard_active = False
            return

        message = f"[Ledger] Drawdown guard triggered — {drawdown:.2%} ≥ {limit:.2%}"
        system_logger.error(message)
        if not self._drawdown_guard_active:
            send_alert(
                "Trading halted: drawdown guard tripped",
                level="CRITICAL",
                context={"drawdown_pct": round(drawdown, 6), "limit_pct": round(limit, 6)},
            )
        self._drawdown_guard_active = True
        raise RuntimeError(message)

    def get_current_drawdown(self) -> float:
        """Return the absolute drawdown observed across recent closed trades."""

        return self._current_max_drawdown()
    ...
    def log_trade(self, trading_pair, trade_size, strategy_name, **kwargs):
        ...
        self._guard_drawdown_limit(limit=0.10)
        ...
        for t in self.trades:
            unique_trades[t.get("trade_id")] = t
        self.trades = list(unique_trades.values())
                ...
                if updated:
                    ...
                    try:
                        if os.path.exists(POSITIONS_PATH):
                            ...
                            with _locked_file(POSITIONS_PATH, "r") as src:
                                with _locked_file(tmp_pos, "w") as dst:
                                    for line in src:
                                        try:
                                            obj = json.loads(line)
    ...
    def reload_trades(self):
        ...
        if os.path.exists(TRADES_LOG_PATH):
            ...
            unique_trades: dict[str, dict] = {}
            for trade in self.trades:
                tid = trade.get("trade_id")
                if tid and tid not in unique_trades:
                    unique_trades[tid] = trade
            self.trades = list(unique_trades.values())
