...
def test_trading_logic_position_manager_check_exits_interface():
    ...
    pm.positions[trade_id] = {
        "trade_id": trade_id, "pair": "BTC/USD", "size": 0.001,
        "entry_price": 20000.0, "timestamp": "2024-01-01T00:00:00+00:00",
        "strategy": "SimpleRSIStrategy", "confidence": 0.5, "high_water_mark": 20000.0,
    }
...
def test_trade_ledger_update_trade_idempotent(tmp_path, monkeypatch):
    ...
    class DummyPM: positions = {}
    tl = ledger_mod.TradeLedger(DummyPM())
    tid = tl.log_trade(trading_pair="BTC/USD", trade_size=0.001, strategy_name="S",
                       confidence=0.5, entry_price=20000.0)
...
def test_rsi_invalid_input_raises():
    ...
    import pytest
    with pytest.raises(ValueError): calculate_rsi([], period=14)
    with pytest.raises(ValueError): calculate_rsi([100.0] * 10, period=14)
